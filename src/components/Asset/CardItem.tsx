...
type CardItemProps = { type: ItemType } & BaseItem;
    ...
    <div className="card glass relative">
      <Link href={`/${type}/${id}`}>
        <figure className="rounded-sm">
          <NextImage className="rounded-xl" width={240} height={360} alt="" src={poster_path} />
        </figure>
        <div className="card-body relative p-1">
          <h4 className="card-title text-[1.1rem]">{title}</h4>
          <p className="text-gray-500 font-light text-sm">{release_date ?? first_air_date}</p>
